...
bool NimBLEAttValue::setValue(const uint8_t* value, uint16_t len) {
    NIMBLE_LOGD(LOG_TAG, "setValue called: len: %d first char: %d : '%c'", len, value[0], value[0]);
    ...
    append(value, len);
    NIMBLE_LOGD(LOG_TAG, "setValue retuns: m_attr_len: %d char: %d : '%c'", m_attr_len, m_attr_value[0], m_attr_value[0]);
